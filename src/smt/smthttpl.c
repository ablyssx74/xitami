    ...
    SYMTAB *config, const char *uri, const char *type, char **base)
{
    ...
    slash = buffer + strlen (buffer);   /*  Start at null at end of URL      */
    while (slash > buffer + 1)
      {
        replaced = *slash;
        *slash = '\0';                  /*  Truncate buffer at slash         */
        alias_value = ini_dyn_value (config, type, buffer + 1, NULL);
        if (alias_value == NULL)
            alias_value = ini_dyn_value (config, type, buffer, NULL);
        *slash = replaced;
    ...
    &&  p_http-> file_size != -1)

      {
        ...
        xstrcat (header,
            ...
            "Keep-alive: ", buffer, eoln,
            NULL);
      }
    if (p_http-> no_cache)
        xstrcat (header,
            "Cache-control: no-cache", eoln,
            "Expires: Thu, 01 Dec 1994 16:00:00 GMT", eoln,
            NULL);
}
 ...
 *  in p_http-> table if any are defined.  Defines a Content-Type header if
 *  none was defined.
 */
        ...
        if (p_http-> file_size != -1)
          {
            ...
            sym_assume_symbol (p_http-> table, "content-length", buffer);
          }
      }
...
process_gsl_script (HTTP_CONTEXT *p_http, XML_ITEM *root_item)
{
    ...
    if (p_http-> ssl_connection)
        ...
        sprintf (port_name, "%d", p_http-> ssl_port);
        
    format_full_url (p_http, buffer, "/", NULL);

    SETXML ("server_software",   server_name);
    SETXML ("server_version",    CUR_VERSION);
